...
<body>
  ...
  <div data-role="page" id="tab1">
    ...
    <div data-role="content">
      <?php $favItemCount = isset($_SESSION['fav']) ? count($_SESSION['fav']) : 0; ?>
      <h3>Tap to Select Three of Your Favorite Dishes</h3>
      <a><?php echo "{$favItemCount}"; ?> of 3</a>
      <?php
        $mysqli = new mysqli("localhost", "id6663956_root","root1","id6663956_categories_db");
        if ($mysqli == false) {
          die("Error: Could not connect. " . mysql_connect_error());
        }
        $dbc = mysqli_connect("localhost", "id6663956_root","root1","id6663956_categories_db");    
              ...
              if ($tmp = $mysqli->query($sql2)) {
                while ($tmp2 = $tmp->fetch_array()) {
                  echo "<li><a href='#'>
                          ...
                          <a href='addToFav.php?id={$tmp2[0]}&name={$tmp2[2]}'>Add to Order</a>
                        </li>";
                }
              }
              echo "</ul></div>";
    ...
    <div data-role="footer" data-position="fixed">
      <div data-role="navbar">
        <ul>
          <li><a href="initialize2.php" data-role="button" data-icon="arrow-r">Next</a></li>
        </ul>
      </div>  
    </div>
  </div> 
